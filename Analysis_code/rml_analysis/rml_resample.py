...
class resample_run:
    def __init__(iso_path, star_path, wrt_path, resample_num, resample_age, Consider_Lumi=True):
        ...
        Names, Mass_star, Mass_star_p_err, Mass_star_m_err, Lumi_star, Lumi_star_p_err, Lumi_star_m_err, Rad_star, Rad_star_p_err, Rad_star_m_err, EEPs = read_candidates(star_path)
...
class resample_star:
    def __init__(iso_path, star_path, wrt_path, resample_age):
        check_file(iso_path)
        check_file(star_path)
        Names, Mass_star, Mass_star_p_err, Mass_star_m_err, Lumi_star, Lumi_star_p_err, Lumi_star_m_err, Rad_star, Rad_star_p_err, Rad_star_m_err, EEPs = read_candidates(star_path)
        mc_num, df_iso, _ = read_iso(iso_path)
